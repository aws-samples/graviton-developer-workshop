...
- Use `eks.amazonaws.com/instance-category` (not `instance-family`) in NodePools
- Enable multi-architecture: both `amd64` and `arm64` in requirements
- Use instance categories (`c`, `m`, `r`) not specific families
- Apply minimum generation constraints using `Gt` operator

## Graviton Instance Targeting
- Target specific Graviton instances at deployment/workload level
- Use `nodeSelector` in deployment manifests for precise targeting
- Do NOT hardcode instance families in NodePool (reduces flexibility)

## Configuration Approaches

### Comparable Instance Approach
- Map x86 to equivalent Graviton generation
- Rule: Add 1 to x86 generation (c6i→c7g, m6i→m7g, r6i→r7g)
- Always validate mapping from AWS documentation first
- If no equivalent exists: Notify user, do not assume
- Keep instance size identical (xlarge→xlarge)
- Provides predictable performance

### Flexible Instances Approach
- Let Karpenter optimize across all Graviton families
- Maximize cost savings through instance diversity
- Allow dynamic selection based on availability and pricing
- Best for cost-sensitive, flexible workloads
...
Follow this phased approach when migrating Karpenter-managed workloads to Graviton.
